	...
	<view class="container">
		...
		<view v-if="!hasLogin || empty === true" class="empty">
			...
			<view v-else class="empty-tips">
				...
				<!-- #ifdef MP-WEIXIN -->
				<button class="login-button" type="primary" open-type="getUserInfo" withCredentials="true" lang="zh_CN" @getuserinfo="wxGetUserInfo">一键快捷登陆</button>
				<!-- #endif -->
			</view>
		</view>
		<view v-else>
				...
				<block v-for="(item, index) in cartList" :key="index">
					<view class="cart-item b-b">
						<view class="image-wrapper">
							<image
								style="opacity: 1;"
								:src="getHeaderImgUrl(item.goodsHeaderImg)"
								mode="aspectFill"
								lazy-load
								@load="onImageLoad('cartList', index)"
								@error="onImageError('cartList', index)"
								@click="toPublicDetail(item)"
							></image>
						...
						<view class="item-right">
							...
							<text class="attr">
								{{ item.selectStyle }}
								<span v-if="item.goodsStatus != 0" style="margin-left: 20rpx;">商品已下架</span>
							</text>
							...
							<uni-number-box
								...
								:min="1"
								:max="item.goodsCount"
								:value="item.number > item.goodsCount ? item.goodsCount : item.number"
								:isMax="item.number >= item.goodsCount ? true : false"
								:isMin="item.number === 1"
								:index="index"
			...
			<view class="action-section">
				<view class="checkbox"><image :src="allChecked ? '/static/selected.png' : '/static/select.png'" mode="aspectFit" @click="check('all')"></image></view>
