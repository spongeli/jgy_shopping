<template>
	<view class="container">
		...
		<!-- 头部轮播 -->
		<head-swiper :carouselList="carouselList"></head-swiper>

		<!-- 首页活动栏目 -->
		<view class="cate-section">
			<block v-for="(item, index) in activeList" :key="index">
				<view class="cate-item" @click="toActivePage(item.url)">
		...
		<block v-for="(item, index) in recommendList" :key="index">
			<view class="ad-1"><image :src="item.src" mode="scaleToFill"></image></view>
		</block>
...
export default {
	...
	methods: {
		...
		// 活动跳转页面
		toActivePage(dest) {
			console.log('点击了活动页面的:' + dest);
			// uni.navigateTo({
			//     url: dest
			// });
		},
		// 请求轮播图数据
		async requestSwiperData() {
		...
		// 请求活动专区信息
		async requestActivityData() {
			let carouselList = await this.$api.json('activeList');
			this.activeList = carouselList || [];
		},
		// 推荐导航栏目
		async requestrecommendData() {
			let recommendList = await this.$api.json('recommendList');
...
/* 分类 */
.cate-section > .cart-item:nth-child(5n + 5) {
